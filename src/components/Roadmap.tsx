...
type PhaseStatus = 'done' | 'live' | 'in-build' | 'coming-soon' | 'planned' | 'future' | 'research';
...
const phases: {
...
}[] = [
  {
    ...
    desc: 'Wallet connection via Hyperliquid RPC, read-only on-chain indexer, first-generation Trader Passport, basic analytics, and public trader profiles.',
    status: 'in-build',
    statusLabel: 'In Build',
    timeline: 'Active',
    active: true,
    milestones: [
      'Hyperliquid RPC integration',
      'Trader Passport schema v1',
      'Public profile pages',
      'Basic PnL + drawdown analytics',
    ],
  },
  {
    num: '02',
    ...
    active: false,
    milestones: [
      'Follow / watchlist system',
      'Achievement badge engine',
      'Score-ranked leaderboards',
  ...
  },
  {
    ...
    active: false,
    milestones: [
      '10-dimension Score engine',
      'AI Trading Journal (GPT-4 powered)',
  ...
  },
  {
    ...
    milestones: [
      ...
      'Private watchlists + filters',
      'Verified trader discovery',
  ...
  },
  {
    ...
    active: false,
    milestones: [
      'Score-gated competition entry',
      'Risk-adjusted challenge format',
      'Partner-sponsored seasons',
      'Performance incentive layer',
    ],
  },
  {
    num: '06',
    ...
    milestones: [
      ...
      'Eligibility tier design',
      'Mandate-based allocation workflows',
      'Protocol fee model',
    ],
  },
];
    ...
    <section id="roadmap" className="py-28 md:py-36 border-t border-protocol-border bg-protocol-bg px-4 sm:px-6">
      <div className="max-w-4xl mx-auto">
        <SectionHeader
          ...
          subtitle="From Reputation MVP to Social Layer, Intelligence Engine, Capital Discovery, Tournaments, and the future Vault Layer."
        />
        ...
        <p className="font-mono text-[9px] text-protocol-text-dim/40 uppercase tracking-wider text-center -mt-8 mb-14 max-w-xl mx-auto">
          Timeline is milestone-gated — not calendar-based — and may evolve based on product, legal, security, and ecosystem readiness.
        </p>
            ...
            {phases.map((p, i) => (
              <motion.div
                ...
                className={`group flex gap-6 md:gap-8 items-start ${
                  !p.active && i > 1 ? 'opacity-50 hover:opacity-100 transition-opacity duration-300' : ''
                }`}
              >
                {/* Timeline dot */}
                <div className="shrink-0 hidden md:flex flex-col items-center gap-2 pt-1">
                  <div
                    className={`w-9.5 h-9.5 rounded-full border flex items-center justify-center ${
                      p.active ? 'border-green/50 bg-green/10' : 'border-protocol-border bg-protocol-bg'
                    }`}
                  >
                    <div
                      className={`w-2.5 h-2.5 rounded-full ${
                        p.active ? 'bg-green animate-pulse' : 'bg-protocol-border'
                      }`}
                    />
                  </div>
                </div>
                ...
                <div
                  className={`flex-1 border p-5 sm:p-6 group-hover:border-blue/30 transition-colors duration-300 relative overflow-hidden ${
                    p.active
                      ...
                      : 'border-protocol-border bg-protocol-accent-bg'
                  }`}
                >
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                    <div className="flex items-center gap-3">
                      <span className="font-display text-[20px] text-protocol-text-dim/30">{p.num}</span>
                      <div>
                        <div className="font-mono text-[9px] uppercase tracking-[0.3em] text-protocol-text-dim/50">
                          {p.meta}
                        </div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      <StatusTag variant={p.status} label={p.statusLabel} pulse={p.active} />
                  ...
                  <p className="text-[13px] text-protocol-text-dim leading-relaxed mb-4">{p.desc}</p>
